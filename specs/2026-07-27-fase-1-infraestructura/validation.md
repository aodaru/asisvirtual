...
- [ ] `.env.example` existe, documenta todas las variables del stack oficial
      de Supabase y **no contiene secretos reales**.
- [ ] `.env` está listado en `.gitignore` y no aparece en `git status`.
- [ ] `db/migrations/001_*.sql` y `db/migrations/002_*.sql` existen y son
...
- [ ] `\dn` lista el schema `asisvirtual`.
- [ ] `\dt asisvirtual.*` lista exactamente `tasks`, `time_entries`,
      `email_logs`.
- [ ] Constraints verificados con pruebas negativas:
...
- [ ] `duration` en `time_entries` se calcula automáticamente al insertar
      `start_time`/`end_time`.
...
- [ ] Entrada de cierre en `progress/history.md` con resultado `completado`.
- [ ] Ningún archivo fuera del alcance fue modificado.
- [ ] Roadmap Fase 1 (1.1–1.5) satisfecho: 1.1 y 1.2 vía despliegue del
